...
const ETYPES := {
	...
	"lord": {n="恐惧领主 · 瓦拉克", body=0x3b1030, skin=0x8a2050, leg=0x210a1c, helm=0xb0303c, weapon=0xe0503c, wkind="sword", aura=0xff3a3a, hp=3100, dmg=[44,64], speed=3.6, range=8.5, xp=3000, scale=2.2, atkCd=1.3, boss=true, ranged=true, skill="slam", skills=["slam","nova"], enrage=true, pcolor=0xff3a2a},
	...
	"singer": {n="无名唱者", body=0xc8d4de, skin=0xe8eef4, leg=0x8aa0b0, aura=0xdfeaff, hp=4400, dmg=[42,60], speed=3.4, range=8.0, xp=4500, scale=2.1, atkCd=1.3, boss=true, ranged=true, skill="nova", skills=["nova","summon"], enrage=true, pcolor=0xdfeaff},
	...
	"gray": {n="凿船者 · 格雷", body=0x4a3a30, skin=0xa89070, leg=0x2a221c, helm=0x5a4a3a, weapon=0xc8b090, wkind="sword", aura=0xc8a060, hp=5400, dmg=[48,70], speed=3.3, range=3.2, xp=5200, scale=2.0, atkCd=1.35, boss=true, skill="slam", skills=["slam","volley"], enrage=true},
	...
	"voice": {n="众议之声", body=0x3a3a4a, skin=0x8a8a9a, leg=0x222230, aura=0xc8c0ff, hp=6000, dmg=[52,76], speed=3.5, range=9.0, xp=6800, scale=2.0, atkCd=1.25, boss=true, ranged=true, skill="nova", skills=["nova","curse","summon"], enrage=true, pcolor=0xc8c0ff},
	"kor": {n="钉底 · 柯尔", body=0x6a4a3a, skin=0xc8a080, leg=0x3a2a22, helm=0x8a7a5a, weapon=0xc0b090, wkind="sword", aura=0xffa060, hp=8600, dmg=[62,94], speed=3.5, range=4.2, xp=8000, scale=1.6, atkCd=1.3, boss=true, skill="slam", skills=["slam","volley","beam","curse","summon"], enrage=true, enrageStrong=true}
}
...
# ---- 武器 GLB 体系（角色手持 + 掉落表现） ----
# 形状 -> 模型 id（对应 godot/models/wpn_*.tscn -> src/wpn_*.glb）
const WPN_SHAPES := {
...
# 怪物 wkind -> 同系形状池（按池随机取，增加持武变化）
const WPN_WKIND_POOL := {
	"sword": ["sword", "axe", "mace", "spear", "dagger"],
	"staff": ["staff", "wand"],
	"bow": ["bow", "crossbow"],
}
# 各形状的握把微调：pos 局部偏移、rot 度、scl 整体缩放（模型 Y 向上、握把在原点）
const WPN_GRIP := {
