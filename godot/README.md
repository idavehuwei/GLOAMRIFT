...
场景外观对齐网页：野外阻挡物（树/尖岩/冰柱/沉船/乱石）、地牢墙帽与柱、地标、石桥镇房屋。阻挡物与地牢墙用 MultiMesh 合批，切图立刻释放旧节点、玩家模型复用。掉落与投射物有 mesh，敌人有血条，NPC 有名字。每座据点从城门走进（石桥从桥头），广场有复活井：死亡在井边醒，也可喝水补满。章节终局 Boss 用 Mixamo 模：瓦拉克 `boss_warrok`、众议之声 `boss_caster`、柯尔 `boss_undead`。
