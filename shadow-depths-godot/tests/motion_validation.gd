...
func run():
	...
	if transition_model.has_meta("authored"):
		var anim = transition_model.get_node("AnimationPlayer")
		check(anim != null and anim.has_animation("attack"),"Authored warrior exposes its attack animation")
		check(anim.has_animation("death"),"Authored warrior exposes its death animation")
	else:
	...
	for i in kinds.size():
		...
		var authored = model.has_meta("authored")
		var anim_player = model.get_node_or_null("AnimationPlayer") if authored else null
		if authored: check(anim_player != null,"Authored model ships an animation player")
		for a in 8:
			var action = w.motion.ACTIONS[a]; w.preview_action = action
			if authored:
				if anim_player.has_animation(action): anim_player.play(action,0.1)
			else:
		...
		if kinds[i]==-11 and not authored:
		...
		if kinds[i] in [4,5]:
			...
			if authored: check(anim_player.get_animation_list().size()>=5,"Authored humanoid ships a full animation set")
			else: check(model.has_node("Head") and model.has_node("LeftLeg/Knee") and model.has_node("SwordArm/Elbow"),"Articulated humanoid hierarchy")
	...
	if smith.has_meta("authored"):
		var sanim = smith.get_node_or_null("AnimationPlayer")
		check(sanim == null or sanim.has_animation("idle") or sanim.get_animation_list().size()>0,"Authored smith has animations")
	else:
